// Minimal driver for the ik_llama.cpp step3vl vision tower.
//
// For each input image, clip_image_preprocess produces a BATCH of preprocessed
// f32 images: the 728px overview plus (for large/high-aspect inputs) any 504px
// slice crops. clip_image_slice_encode encodes the views (one graph each, inside
// libmtmd) and we dump the post-projector embeddings as ik_embeddings.json.
#include "clip.h"
...
static const char * IK_PRE_DIR = "/tmp/step3vl_pre";
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (std::string(argv[i]) == "--cpu") {
            use_cpu = true;
        } else if (std::string(argv[i]) == "--ik-image-preprocessing") {
    ...
    for (size_t i = 1; i < args.size(); i++) {
        ...
        const int n_imgs = (int) clip_image_f32_batch_n_images(batch);

        // preprocess batch order: entry 0 is the overview, entries 1..N-1 are
        // the slice crops (already in model order among themselves)
        std::vector<int>         view_tokens(n_imgs);
        std::vector<std::string> view_names(n_imgs);
        int n_tokens = 0;
        for (int b = 0; b < n_imgs; b++) {
            clip_image_f32 * f32 = clip_image_f32_get_img(batch, b);
            view_tokens[b] = clip_n_output_tokens(ctx, f32);
            view_names[b]  = b == 0 ? "overview" : "slice" + std::to_string(b - 1);
            n_tokens += view_tokens[b];

            // --ik-image-preprocessing: dump each preprocessed (resized+
            // normalized, interleaved RGB row-major) f32 image for parity checks
            // against the HF processor and for ref_encode.py's encoder-only mode.
            if (ik_image_preprocessing) {
                const std::string dpath = std::string(IK_PRE_DIR) + "/" + basename_of(path) +
                    "." + view_names[b] + ".pre.bin";
        ...
        if (!clip_image_slice_encode(ctx, n_threads, batch, vec.data())) {
            fprintf(stderr, "encode failed for %s\n", path.c_str());
            return 1;
        }

        const std::string base = basename_of(path);
        const float * row = vec.data();
        for (int b = 0; b < n_imgs; b++) {
            // [view_tokens x n_embd]: one base64 string per token row, each the
            // row's n_embd raw little-endian f32 bytes. See tests/mtmd-encoder/README.md.
            nlohmann::json rows = nlohmann::json::array();
            for (int t = 0; t < view_tokens[b]; t++) {
                const float * r = row + (size_t) t * n_embd;
                rows.push_back(base64::encode(reinterpret_cast<const char *>(r),
                                              (size_t) n_embd * sizeof(float)));
            }
            out[base + "." + view_names[b]] = std::move(rows);
            row += (size_t) view_tokens[b] * n_embd;
        }
