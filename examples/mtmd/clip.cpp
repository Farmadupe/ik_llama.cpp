...
// declarations only; STB_IMAGE_RESIZE_IMPLEMENTATION lives in mtmd-helper.cpp
...
private:
    ...
    // Bicubic resize via stb_image_resize2 (separable + SIMD).
    // CATMULLROM is an interpolating cubic spline; pixels are treated as linear
    // (no sRGB gamma), with clamped edges, matching the previous resampler.
    static bool resize_bicubic(const clip_image_u8 & src, clip_image_u8 & dst, int target_width, int target_height) {
        dst.nx = target_width;
        dst.ny = target_height;
