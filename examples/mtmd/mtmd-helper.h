...
// convenience wrapper around the embd stream below: encodes and decodes the
// whole chunk list against lctx, handling batching and non-causal media
// rounds. Returns 0 on success and forwards the first encode/decode error
// otherwise.
...
// Resumable embedding stream over a list of input chunks. Text chunks have
// their embeddings looked up via the model's input-embedding LUT; media
// chunks are encoded lazily via mtmd_encode_chunk_into.
//
// Each drain call fills up to n_max embedding rows (plus positions, seq ids
// and logits flags, all cleared to false) into a caller-provided staging
// batch. The stream can suspend anywhere, including in the middle of an
// image. It never calls llama_decode and never toggles causal attention;
// the caller owns both.
//
// Text chunks are copied at init and need only outlive the init call;
// media chunks are borrowed and must outlive the stream.
//
// this API is NOT thread-safe
typedef struct mtmd_helper_embd_stream mtmd_helper_embd_stream;

// returns nullptr on invalid input (chunk of an unknown type)
