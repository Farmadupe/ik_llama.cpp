...
struct server_context {
    ...
    // embd staging for rounds that carry multimodal rows (allocated in
    // init() iff mctx). A round is either a token round (the llama_batch
    // above, byte-identical to the text-only server) or an embd round
    // (token rows converted through llama_input_embeddings plus media rows
    // drained from per-slot streams, staged here).
    ...
    bool    round_is_embd    = false; // flavor of the current update_slots round
