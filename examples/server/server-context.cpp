...
void server_context::batch_pending_prompt(const int32_t n_ubatch, const int32_t n_batch,  int32_t & batch_type) {
            ...
            if (slot.state == SLOT_STATE_IDLE && slot.command == SLOT_COMMAND_LOAD_PROMPT) {
                ...
                if (mctx != nullptr && slot.n_past_prompt < slot.n_prompt_tokens
                    ...
                    if (!slot.mm_stream) {
                        ...
                        auto descriptors = slot.prompt_tokens.build_coalesce_descriptors(
                                slot.n_past_prompt, slot.n_prompt_tokens);
                        slot.mm_stream = mtmd_helper_embd_stream_init(
                                mctx, ctx, descriptors.data(), descriptors.size(), p1, slot.id);
                        if (!slot.mm_stream) {
    ...
    // Drain multimodal streams into this round. If any stream rows land, the
    // round becomes an embd round: the token rows staged above (generation +
    // text prefill) are converted through the input-embedding LUT and the
    // stream rows follow. With no stream rows this is a token round,
    // byte-identical to the text-only server.
    round_is_embd = false;
    round_n_rows  = batch.n_tokens;
    if (!drain_pending.empty() && batch.n_tokens < n_batch) {
        ...
        if (row > n_tok) {
            bool converted = true;
            if (n_tok > 0) {
                // convert the token rows staged above; row indices are preserved
                // 1:1, so slot.i_batch / i_batch_dft / prompt_batch_i0/i1 stay valid
                int32_t res = llama_input_embeddings(ctx, batch.token, n_tok, embd_data.data(),
                                                     llama_model_n_embd(model));
                if (res != 0) {
                    // cannot build the embd round; drop this round's stream rows
                    // (nothing was decoded) and fall back to the token round
                    LLAMA_LOG_ERROR("llama_input_embeddings failed, dropping multimodal rows this round\n");
                    converted = false;
                    for (server_slot * slot_ptr : drain_pending) {
                        server_slot & slot = *slot_ptr;
                        if (slot.mm_mirror_pending) {
                            slot.release();
                            send_error(slot, "multimodal prefill failed", ERROR_TYPE_SERVER);
                        }
                    }
                } else {
...
void server_context::process_batch_tokens(int32_t & n_batch) {
    for (int32_t i = 0; i < round_n_rows; i += n_batch) {
        ...
        if (round_is_embd) {
            // embd round: rows staged in batch_embd (get_view re-strides the
            // M-RoPE position planes for the sub-range)
            batch_view = batch_embd->get_view(i, n_tokens);
        } else {
            batch_view = {
                n_tokens,
                batch.token + i,
    ...
    if (server_speculative_uses_target_features(params_base.speculative)) {
        ...
        for (auto & slot : slots) {
            ...
            if (consumed > 0) {
                ...
                const double eta_sec = (slot.n_prompt_tokens - slot.n_past_prompt) / (tps + 1e-9);
