    ...
    request_trace* trace)
{
    ...
    auto fetch_image_bytes = [&](const std::string & url) -> raw_buffer {
    ...
    for (auto& msg : messages) {
        ...
        for (auto& p : content) {
            ...
            if (type == "image_url") {
                ...
                out_files.push_back(fetch_image_bytes(url));
...
server_tokens::chunk_view server_tokens::build_eval_chunks(
    size_t start_token_idx, size_t end_token_idx) const {
    chunk_view view;
    if (start_token_idx >= end_token_idx || end_token_idx > tokens.size()) {
        return view;
    }
    view.chunks.reserve(map_idx_to_media.size() * 2 + 1);

    size_t idx = start_token_idx;
    while (idx < end_token_idx) {
        auto media_it = map_idx_to_media.find(idx);
        if (media_it != map_idx_to_media.end()) {
            const mtmd_input_chunk * chunk = media_it->second.get();
            view.chunks.push_back(chunk);
            idx += mtmd_input_chunk_get_n_tokens(chunk);
        } else {
            // text run: spans from idx up to the next media boundary or end_token_idx.
            auto next_media = map_idx_to_media.lower_bound(idx);
            size_t text_end = (next_media != map_idx_to_media.end()) ? next_media->first : tokens.size();
            if (text_end > end_token_idx) {
                text_end = end_token_idx;
            }
            view.text_owner.emplace_back(mtmd_input_chunk_init_text(tokens.data() + idx, text_end - idx));
            view.chunks.push_back(view.text_owner.back().get());
            idx = text_end;
        }
    }
    return view;
